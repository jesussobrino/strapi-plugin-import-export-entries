...
export default {
  register(app) {
    app.addMenuLink({
      ...
      Component: async () => {
        const component = await import(/* webpackChunkName: "import-export-entries-js" */ './pages/App');
  ...
  async registerTrads({ locales }) {
    const importedTrads = await Promise.all(
      locales.map((locale) => {
        return import(/* webpackChunkName: "import-export-entries-js-translation-[request]" */ `./translations/${locale}.json`)
          .then(({ default: data }) => {
            return {
